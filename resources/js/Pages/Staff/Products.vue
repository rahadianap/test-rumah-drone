                                    ...
                                    <table class="min-w-full border divide-y divide-gray-200">
                                        ...
                                        <tbody class="bg-white divide-y divide-gray-200 divide-solid">
                                            <tr class="bg-white" v-for="(item, index) in products.data" :key="index">
                                                ...
                                                <td class="px-6 py-4 text-sm leading-5 text-gray-900 whitespace-no-wrap">
                                                    {{ item.product_initial_stock.toLocaleString('id') }}
                                                </td>
                                                <td class="px-6 py-4 text-sm leading-5 text-gray-900 whitespace-no-wrap">
                                                    {{ item.product_stock.toLocaleString('id') }}
                                                </td>
                                                <td class="px-6 py-4 text-sm leading-5 text-gray-900 whitespace-no-wrap">
                                                    {{ item.product_min_stock.toLocaleString('id') }}
                                                </td>
                                                <!-- <td class="px-6 py-4 text-sm leading-5 text-gray-900 whitespace-no-wrap">
            ...
            await axios.get(this.url).then(response => {
                ...
                for(let i = 0; i < this.products.data.length; i++){
                    if(this.products.data.some(x => this.products.data[i]['product_stock'] <= this.products.data[i]['product_min_stock'])) {
                        this.$toast.warning('Stok Produk: ' + this.products.data[i]['product_name'] + ' Mendekati Minimum Stok.', {
                            position: 'top',
                            duration: 0,
                            dismissible: true
                        });
